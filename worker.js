...
{"options":[{"name":"meal name","description":"short description","ingredients":["ingredient1","ingredient2","ingredient3","ingredient4"],"kcalPerPerson":0,"carbsPerPerson":0,"proteinPerPerson":0}],"translatedFridge":{"hebrewItem":"english",...}}`;
        ...
        prompt = isHeDp
          ? `תכנון יומי אישי.${dietRuleHe ? `\n⚠️ הגבלות תזונה — חלות על כל רעיון בלי יוצא מן הכלל:\n${dietRuleHe}` : ''}\nפרופיל: גיל ${dp?.age||'?'}, ${gHe}${bmi?`, BMI ${bmi}`:''}, מצבים: ${condLine}, יעד יומי: ${tKcal}קק״ל / ${tCarbs}g פחמ׳ / ${tProt}g חלבון. היסטוריה(${history?.days||0} ימים): ממוצע ${history?.avgKcal||0}קק״ל, מזונות: ${foodList||'אין'}.${sugarNote} ${guideHe}החזר JSON (targetCarbs חייב להסתכם ב-${tCarbs}g):\n{"meals":[{"type":"breakfast","label":"ארוחת בוקר","time":"07:00-09:00","targetKcal":${mKcal[0]},"targetCarbs":${mCarbs[0]},"targetProtein":${mProt[0]},"targetFat":${mFat[0]},"ideas":["דייסת שיבולת שועל עם פירות","לחם מחיטה מלאה עם טחינה ואבוקדו","שייק עם בננה ושקדים"],"note":""},{"type":"morning_snack","label":"ביניים בוקר","time":"10:30-11:00","targetKcal":${mKcal[1]},"targetCarbs":${mCarbs[1]},"targetProtein":${mProt[1]},"targetFat":${mFat[1]},"ideas":["תפוח עם חופן אגוזים","גזר עם חומוס"],"note":""},{"type":"lunch","label":"ארוחת צהריים","time":"12:30-14:00","targetKcal":${mKcal[2]},"targetCarbs":${mCarbs[2]},"targetProtein":${mProt[2]},"targetFat":${mFat[2]},"ideas":["קינואה עם קטניות וירק צלוי","עדשים עם אורז וסלט","פלאפל עם חומוס"],"note":""},{"type":"afternoon_snack","label":"ביניים אחה\"צ","time":"16:00-17:00","targetKcal":${mKcal[3]},"targetCarbs":${mCarbs[3]},"targetProtein":${mProt[3]},"targetFat":${mFat[3]},"ideas":["פרי עם שקדים","חומוס עם ירק"],"note":""},{"type":"dinner","label":"ארוחת ערב","time":"19:00-20:30","targetKcal":${mKcal[4]},"targetCarbs":${mCarbs[4]},"targetProtein":${mProt[4]},"targetFat":${mFat[4]},"ideas":["מרק עדשים עם ירק","סלט גדול עם גרגירי חומוס","טופו מוקפץ עם ירק ואורז"],"note":""}],"substitutions":[{"original":"מזון חוזר","suggested":"תחליף","reason":"סיבה"}],"insight":"תובנה קצרה","totalKcal":${tKcal},"totalCarbs":${tCarbs},"totalProtein":${tProt},"totalFat":${tFat}}`
          : `Daily plan.${dietRuleEn ? `\n⚠️ DIETARY RESTRICTIONS — apply to EVERY idea, no exceptions:\n${dietRuleEn}` : ''}\nProfile: age ${dp?.age||'?'}, ${dp?.gender||'?'}${bmi?`, BMI ${bmi}`:''}, conditions: ${condLine}, daily targets: ${tKcal}kcal / ${tCarbs}g carbs / ${tProt}g protein. History(${history?.days||0}d): avg ${history?.avgKcal||0}kcal, foods: ${foodList||'none'}.${sugarNote} ${guideEn}Return JSON (targetCarbs must total ${tCarbs}g):\n{"meals":[{"type":"breakfast","label":"Breakfast","time":"7:00-9:00","targetKcal":${mKcal[0]},"targetCarbs":${mCarbs[0]},"targetProtein":${mProt[0]},"targetFat":${mFat[0]},"ideas":["Oatmeal with berries and nuts","Whole grain toast with avocado and tahini","Smoothie with banana and almond butter"],"note":""},{"type":"morning_snack","label":"Morning Snack","time":"10:30-11:00","targetKcal":${mKcal[1]},"targetCarbs":${mCarbs[1]},"targetProtein":${mProt[1]},"targetFat":${mFat[1]},"ideas":["Apple with almonds","Carrot with hummus"],"note":""},{"type":"lunch","label":"Lunch","time":"12:30-2:00","targetKcal":${mKcal[2]},"targetCarbs":${mCarbs[2]},"targetProtein":${mProt[2]},"targetFat":${mFat[2]},"ideas":["Quinoa with legumes and roasted veg","Lentils with rice and salad","Falafel with hummus"],"note":""},{"type":"afternoon_snack","label":"Afternoon Snack","time":"4:00-5:00","targetKcal":${mKcal[3]},"targetCarbs":${mCarbs[3]},"targetProtein":${mProt[3]},"targetFat":${mFat[3]},"ideas":["Fruit with nuts","Hummus with raw vegetables"],"note":""},{"type":"dinner","label":"Dinner","time":"7:00-8:30","targetKcal":${mKcal[4]},"targetCarbs":${mCarbs[4]},"targetProtein":${mProt[4]},"targetFat":${mFat[4]},"ideas":["Lentil soup with vegetables","Large salad with chickpeas and walnuts","Stir-fried tofu with vegetables and rice"],"note":""}],"substitutions":[{"original":"food","suggested":"alternative","reason":"reason"}],"insight":"brief insight","totalKcal":${tKcal},"totalCarbs":${tCarbs},"totalProtein":${tProt},"totalFat":${tFat}}`;
      } else if (profileData) {
        model = 'claude-sonnet-4-6';
        max_tokens = 2000;
