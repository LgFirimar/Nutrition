...
{"options":[{"name":"meal name","description":"short description","ingredients":["ingredient1","ingredient2","ingredient3","ingredient4"],"kcalPerPerson":0,"carbsPerPerson":0,"proteinPerPerson":0}],"translatedFridge":{"hebrewItem":"english",...}}`;
        ...
        max_tokens = 3000;
        system = isHeDp
          ? 'את תזונאית קלינית, אנדוקרינולוגית ורופאת משפחה מומחית. את בקיאה בהנחיות האיגודים הישראליים (האיגוד הישראלי לסוכרת, IDA), ADA, AACE, NAMS, EMAS, ACC/AHA, WHO, Health Canada ו-NHS UK. ענה בעברית בלבד. החזר JSON בלבד, ללא markdown.'
          : 'You are a clinical nutritionist, endocrinologist and family medicine specialist. You follow guidelines from ADA, AACE, NAMS, EMAS, ACC/AHA, WHO, Health Canada, NHS UK. Respond in English only. Return ONLY valid JSON, no markdown.';
        ...
        const foodList = (history?.topFoods || []).slice(0, 12).map(f => `${f.food} (${f.count}×)`).join(', ');
        const sugarLine = history?.sugarAvg ? (isHeDp ? `ממוצע סוכר: ${history.sugarAvg} mg/dL, טווח: ${history.sugarMin}-${history.sugarMax}` : `Avg blood sugar: ${history.sugarAvg} mg/dL, range: ${history.sugarMin}-${history.sugarMax}`) : '';
        const condLine = (dp?.conditions || []).join(', ') || (isHeDp ? 'אין' : 'none');
        const goalLine = (dp?.goals || []).join(', ') || (isHeDp ? 'בריאות כללית' : 'general health');
        prompt = isHeDp
          ? `בני תכנון ארוחות יומי מותאם אישית לחלוטין.\n\n**נתוני המשתמשת:**\nגיל: ${dp?.age||'לא צוין'} | מין: ${gHe} | גובה: ${dp?.height||'?'}ס"מ | משקל: ${dp?.weight||'?'}ק"ג${bmi?` | BMI: ${bmi}`:''}\nמצבים רפואיים: ${condLine}\nיעדים: ${goalLine}\nהעדפות תזונה: ${(dp?.dietPrefs||[]).join(', ')||'אין'}\nפעילות גופנית: ${dp?.activityText||dp?.activity||'מתון'}\nיעדי תזונה יומיים: ${dp?.maxKcal||1800} קק״ל | פחמימות: ${dp?.maxCarbs||130}g | חלבון: ${dp?.maxProtein||90}g\n\n**היסטוריית אכילה (30 ימים אחרונים):**\nמזונות שחוזרים: ${foodList||'אין נתונים'}\nממוצע יומי בפועל: ${history?.avgKcal||0} קק״ל | פחמ׳: ${history?.avgCarbs||0}g | חלבון: ${history?.avgProtein||0}g | שומן: ${history?.avgFat||0}g\nימים עם מעקב: ${history?.days||0}\n${sugarLine}\n\n**הוראות:**\n1. חלקי את היום ל-5 ארוחות (בוקר, ביניים בבוקר, צהריים, ביניים אחה"צ, ערב)\n2. לכל ארוחה: הגדירי חלון זמן, יעדי מאקרו, 2-3 רעיונות קונקרטיים מותאמים לטעמי המשתמשת\n3. בסיסי על ההנחיות: ${(dp?.conditions||[]).includes('t2d')||(dp?.conditions||[]).includes('prediab')?'ADA 2024: 45-60g פחמ׳ לארוחה, GI נמוך, העדפה לסיבים. ':''}${(dp?.conditions||[]).includes('menopause')?'NAMS 2023: חלבון גבוה 1.2g/kg, סידן 1200mg, ויט D, פיטואסטרוגנים. ':''}${(dp?.conditions||[]).includes('heart')?'AHA/ACC: דיאטה ים-תיכונית, שומן רווי <10%, אומגה 3. ':''}WHO: מזון עשיר בסיבים, מינימום סוכר מוסף.\n4. זהי עד 3 מזונות חוזרים שפחות מיטיבים — הצעי תחליף ספציפי עם הסבר קצר\n5. כתבי תובנה אחת על דפוס האכילה של המשתמשת\n\nהחזרי JSON בלבד:\n{"meals":[{"type":"breakfast","label":"ארוחת בוקר","time":"07:00–09:00","targetKcal":0,"targetCarbs":0,"targetProtein":0,"targetFat":0,"ideas":["רעיון 1","רעיון 2","רעיון 3"],"note":""},{"type":"morning_snack","label":"ארוחת ביניים – בוקר","time":"10:00–11:00","targetKcal":0,"targetCarbs":0,"targetProtein":0,"targetFat":0,"ideas":["רעיון 1","רעיון 2"],"note":""},{"type":"lunch","label":"ארוחת צהריים","time":"12:30–14:00","targetKcal":0,"targetCarbs":0,"targetProtein":0,"targetFat":0,"ideas":["רעיון 1","רעיון 2","רעיון 3"],"note":""},{"type":"afternoon_snack","label":"ארוחת ביניים – אחה״צ","time":"16:00–17:00","targetKcal":0,"targetCarbs":0,"targetProtein":0,"targetFat":0,"ideas":["רעיון 1","רעיון 2"],"note":""},{"type":"dinner","label":"ארוחת ערב","time":"18:30–20:00","targetKcal":0,"targetCarbs":0,"targetProtein":0,"targetFat":0,"ideas":["רעיון 1","רעיון 2","רעיון 3"],"note":""}],"substitutions":[{"original":"מאכל חוזר","suggested":"תחליף מיטבי","reason":"הסבר קצר"}],"insight":"תובנה אחת על דפוס האכילה","totalKcal":0,"totalCarbs":0,"totalProtein":0,"totalFat":0}`
          : `Build a fully personalized daily meal plan.\n\n**User Profile:**\nAge: ${dp?.age||'N/A'} | Gender: ${dp?.gender||'N/A'} | Height: ${dp?.height||'?'}cm | Weight: ${dp?.weight||'?'}kg${bmi?` | BMI: ${bmi}`:''}\nMedical conditions: ${condLine}\nGoals: ${goalLine}\nDiet preferences: ${(dp?.dietPrefs||[]).join(', ')||'none'}\nActivity: ${dp?.activityText||dp?.activity||'moderate'}\nDaily targets: ${dp?.maxKcal||1800} kcal | Carbs: ${dp?.maxCarbs||130}g | Protein: ${dp?.maxProtein||90}g\n\n**Eating History (last 30 days):**\nRecurring foods: ${foodList||'no data'}\nDaily averages: ${history?.avgKcal||0} kcal | Carbs: ${history?.avgCarbs||0}g | Protein: ${history?.avgProtein||0}g | Fat: ${history?.avgFat||0}g\nDays tracked: ${history?.days||0}\n${sugarLine}\n\n**Instructions:**\n1. Split into 5 meals (breakfast, morning snack, lunch, afternoon snack, dinner)\n2. For each: time window, macro targets, 2-3 concrete food ideas\n3. Apply guidelines: ${(dp?.conditions||[]).includes('t2d')||(dp?.conditions||[]).includes('prediab')?'ADA 2024: 45-60g carbs/meal, low GI, high fiber. ':''}${(dp?.conditions||[]).includes('menopause')?'NAMS 2023: 1.2g/kg protein, Ca 1200mg, vit D, phytoestrogens. ':''}${(dp?.conditions||[]).includes('heart')?'AHA/ACC: Mediterranean, sat fat <10%, omega-3. ':''}WHO: high fiber, minimal added sugar.\n4. Identify up to 3 suboptimal recurring foods — suggest better alternatives\n5. Write one insight about eating pattern\n\nReturn ONLY JSON:\n{"meals":[{"type":"breakfast","label":"Breakfast","time":"7:00–9:00 AM","targetKcal":0,"targetCarbs":0,"targetProtein":0,"targetFat":0,"ideas":["idea 1","idea 2","idea 3"],"note":""},{"type":"morning_snack","label":"Morning Snack","time":"10:00–11:00 AM","targetKcal":0,"targetCarbs":0,"targetProtein":0,"targetFat":0,"ideas":["idea 1","idea 2"],"note":""},{"type":"lunch","label":"Lunch","time":"12:30–2:00 PM","targetKcal":0,"targetCarbs":0,"targetProtein":0,"targetFat":0,"ideas":["idea 1","idea 2","idea 3"],"note":""},{"type":"afternoon_snack","label":"Afternoon Snack","time":"4:00–5:00 PM","targetKcal":0,"targetCarbs":0,"targetProtein":0,"targetFat":0,"ideas":["idea 1","idea 2"],"note":""},{"type":"dinner","label":"Dinner","time":"6:30–8:00 PM","targetKcal":0,"targetCarbs":0,"targetProtein":0,"targetFat":0,"ideas":["idea 1","idea 2","idea 3"],"note":""}],"substitutions":[{"original":"food","suggested":"better alternative","reason":"reason"}],"insight":"one insight","totalKcal":0,"totalCarbs":0,"totalProtein":0,"totalFat":0}`;
      } else if (profileData) {
        model = 'claude-sonnet-4-6';
        max_tokens = 2000;
