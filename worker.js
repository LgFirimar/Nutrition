...
{"options":[{"name":"meal name","description":"short description","ingredients":["ingredient1","ingredient2","ingredient3","ingredient4"],"kcalPerPerson":0,"carbsPerPerson":0,"proteinPerPerson":0}],"translatedFridge":{"hebrewItem":"english",...}}`;
        ...
        prompt = isHeDp
          ? `תכנון יומי אישי. פרופיל: גיל ${dp?.age||'?'}, ${gHe}${bmi?`, BMI ${bmi}`:''}, מצבים: ${condLine}, יעד: ${dp?.maxKcal||1800}קק״ל/${dp?.maxCarbs||130}gפחמ׳/${dp?.maxProtein||90}gחלבון. היסטוריה(${history?.days||0} ימים): ממוצע ${history?.avgKcal||0}קק״ל, מזונות: ${foodList||'אין'}.${sugarNote} הנחיות: ${guideHe}מזון עשיר סיבים, פחות סוכר. החזר JSON:\n{"meals":[{"type":"breakfast","label":"ארוחת בוקר","time":"07:00-09:00","targetKcal":400,"targetCarbs":40,"targetProtein":20,"targetFat":10,"ideas":["דייסת שיבולת שועל עם פירות","יוגורט עם גרנולה","ביצים עם ירק"],"note":""},{"type":"morning_snack","label":"ביניים בוקר","time":"10:30-11:00","targetKcal":150,"targetCarbs":15,"targetProtein":8,"targetFat":5,"ideas":["תפוח עם אגוזים","גבינה עם ירק"],"note":""},{"type":"lunch","label":"ארוחת צהריים","time":"12:30-14:00","targetKcal":550,"targetCarbs":50,"targetProtein":35,"targetFat":15,"ideas":["סלמון עם קינואה וירק","עוף עם אורז וסלט","קטניות עם ירק"],"note":""},{"type":"afternoon_snack","label":"ביניים אחה\"צ","time":"16:00-17:00","targetKcal":150,"targetCarbs":15,"targetProtein":8,"targetFat":5,"ideas":["פרי עם גבינה","חומוס עם ירק"],"note":""},{"type":"dinner","label":"ארוחת ערב","time":"19:00-20:30","targetKcal":500,"targetCarbs":40,"targetProtein":30,"targetFat":15,"ideas":["מרק ירקות עם קטניות","סלט גדול עם טונה","ביצים עם ירק מוקפץ"],"note":""}],"substitutions":[{"original":"מזון חוזר","suggested":"תחליף","reason":"סיבה"}],"insight":"תובנה קצרה","totalKcal":1750,"totalCarbs":160,"totalProtein":101,"totalFat":50}`
          : `Daily plan. Profile: age ${dp?.age||'?'}, ${dp?.gender||'?'}${bmi?`, BMI ${bmi}`:''}, conditions: ${condLine}, targets: ${dp?.maxKcal||1800}kcal/${dp?.maxCarbs||130}gcarbs/${dp?.maxProtein||90}gprot. History(${history?.days||0}d): avg ${history?.avgKcal||0}kcal, foods: ${foodList||'none'}.${sugarNote} Guidelines: ${guideEn}high fiber, low sugar. Return JSON:\n{"meals":[{"type":"breakfast","label":"Breakfast","time":"7:00-9:00","targetKcal":400,"targetCarbs":40,"targetProtein":20,"targetFat":10,"ideas":["Oatmeal with berries","Greek yogurt with granola","Eggs with vegetables"],"note":""},{"type":"morning_snack","label":"Morning Snack","time":"10:30-11:00","targetKcal":150,"targetCarbs":15,"targetProtein":8,"targetFat":5,"ideas":["Apple with nuts","Cheese with vegetables"],"note":""},{"type":"lunch","label":"Lunch","time":"12:30-2:00","targetKcal":550,"targetCarbs":50,"targetProtein":35,"targetFat":15,"ideas":["Salmon with quinoa","Chicken with rice and salad","Legumes with vegetables"],"note":""},{"type":"afternoon_snack","label":"Afternoon Snack","time":"4:00-5:00","targetKcal":150,"targetCarbs":15,"targetProtein":8,"targetFat":5,"ideas":["Fruit with cheese","Hummus with vegetables"],"note":""},{"type":"dinner","label":"Dinner","time":"7:00-8:30","targetKcal":500,"targetCarbs":40,"targetProtein":30,"targetFat":15,"ideas":["Vegetable soup with legumes","Large salad with tuna","Eggs with stir-fried vegetables"],"note":""}],"substitutions":[{"original":"food","suggested":"alternative","reason":"reason"}],"insight":"brief insight","totalKcal":1750,"totalCarbs":160,"totalProtein":101,"totalFat":50}`;
      } else if (profileData) {
        model = 'claude-sonnet-4-6';
        max_tokens = 2000;
